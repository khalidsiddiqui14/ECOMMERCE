...
const saveAuthData = (data) => {
  try {
    ...
    const refresh = data?.refresh_token || data?.refresh || data?.tokens?.refresh || data?.data?.refresh_token || null;
    const user = data?.user || data?.data?.user || data?.profile || data?.data?.profile || null;
    if (access) {
...
export const sendLoginOtp = async (emailOrPhone) => {
  ...
  const payload = isEmail ? { email: normalizeEmail(value) } : { phone: normalizePhone(value) };
