...
const Input = ({ label, id, readOnly, small,...props }) => (
  <div className="flex flex-col gap-1">
    <label htmlFor={id} className="text-xs font-bold uppercase text-[#0F1111]">
      {label}
    </label>
    <input
      id={id}
      {...props}
      readOnly={readOnly}
      className={`h-8 px-2 border border-[#a6a6a6] rounded-sm text-sm outline-none ${
        readOnly? "bg-[#f0f2f2]" : "bg-white"
      }`}
    />
...
function Profile() {
  ...
  if (loading)
    return (
      <div className="bg-[#EAEDED] min-h-screen grid place-items-center">
    ...
    <div className="bg-[#EAEDED] min-h-screen py-3">
      <div className="max-w- mx-auto px-2">
        <div className="bg-white border rounded-lg">
          ...
          {message && (
            <div className="m-3 p-2 bg-[#f0f8f0] border border-[#067D62] rounded text-sm">{message}</div>
          )}
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
            ...
            <button type="submit" disabled={saving} className="w-full h-9 bg-[#FFD814] border rounded-lg font-bold">
