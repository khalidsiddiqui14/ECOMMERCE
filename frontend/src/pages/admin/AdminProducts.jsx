...
export default function AdminProducts() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const r = await fetch(API_URL, { headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  const fetchBrandsAndCategories = async () => {
    try {
      ...
      const [brandsRes, categoriesRes, storesData] = await Promise.all([
  ...
  const handleAdd = async (e) => {
    ...
    setMsg({ type: "", text: "" });
    try {
      if (!form.store) throw new Error("Please select a Store. Create a Store from Vendor → Store first.");
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.detail || data.message || Object.entries(data).map(([k,v])=>`${k}: ${Array.isArray(v)?v.join(", "):v}`).join(" | ") || "Product create failed.");
      if (form.images.length) {
        for (let i=0;i<form.images.length;i++) {
          ...
          if (!imageRes.ok) console.warn("Product image upload failed", await imageRes.text().catch(()=>""));
        }
      }
      setMsg({ type: "success", text: `✅ Added! ID:${data.id} Name:${data.name}` });
      previews.forEach(u => URL.revokeObjectURL(u));
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`${API_URL}${id}/`, { method: "DELETE", headers: { Authorization: `Bearer ${getToken()}` } });
    fetchProducts();
  };
      ...
      <div className="max-w- mx-auto">
        <div className="bg-[#232F3E] text-white p-4 rounded-t-lg flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p className="text- mt-1">Total: {products.length} - Store/Brand/Category exact admin name</p>
          </div>
        ...
        <div className="bg-white p-4 border border-t-0 rounded-b-lg">
          ...
          {showAdd && (
            <div className="bg-[#f7fafa] border-2 border-[#FFD814] rounded-lg p-4 mb-6">
              <h2 className="font-bold text- mb-3">➕ Admin - Add Product</h2>
              <form onSubmit={handleAdd} className="space-y-3">
                <div><label className="text- font-bold">Product Name *</label><input name="name" value={form.name} onChange={handleChange} required placeholder="e.g. Samsung Galaxy S24 Ultra" className="w-full mt-1 border border-[#888] rounded-lg px-3 h-10 text-" /></div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text- font-bold">Store *</label>
                    <select name="store" value={form.store} onChange={handleChange} required className="w-full mt-1 border rounded-lg px-3 h-10 text- bg-white">
                      <option value="">Select Store</option>
                      {stores.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                    {stores.length===0 && <p className="text-[11px] text-[#CC0C39] mt-1">No Store available. Vendor must create a Store first.</p>}
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text- font-bold">Brand *</label>
                    ...
                    {showAddBrand && <div className="flex gap-2 mt-2"><input value={newBrand} onChange={e=>setNewBrand(e.target.value)} placeholder="e.g. ASUS" className="flex-1 border rounded-lg px-3 h-9 text-" /><button type="button" onClick={handleAddBrand} disabled={addingBrand} className="px-3 h-9 bg-[#FFD814] border border-[#FCD200] rounded-lg text- font-bold">{addingBrand? "Adding..." : "Add Brand"}</button></div>}
                  </div>

                  <div>
                    <label className="text- font-bold">Category *</label>
                    <div className="flex gap-2 mt-1">
                    ...
                    {showAddCategory && <div className="flex gap-2 mt-2"><input value={newCategory} onChange={e=>setNewCategory(e.target.value)} placeholder="e.g. Mobile Phones" className="flex-1 border rounded-lg px-3 h-9 text-" /><button type="button" onClick={handleAddCategory} disabled={addingCategory} className="px-3 h-9 bg-[#FFD814] border border-[#FCD200] rounded-lg text- font-bold">{addingCategory? "Adding..." : "Add Category"}</button></div>}
                  </div>
                </div>
                ...
                <button disabled={adding || stores.length===0} type="submit" className="w-full h-11 bg-[#FFD814] hover:bg-[#F7CA00] border border-[#FCD200] rounded-lg font-bold text-">{adding? "Adding..." : stores.length===0? "Create Store First" : "Add Product"}</button>
              </form>
            </div>
          )}
          ...
          {loading? <div className="grid grid-cols-4 gap-3">{Array.from({length:8}).map((_,i)=><div key={i} className="h- bg-gray-100 animate-pulse rounded" />)}</div> : (
            <>
              <div className="flex justify-between mb-3"><h2 className="font-bold text-">All Products - {products.length}</h2><span className="text- bg-[#f0f2f2] px-2 py-1 rounded">{BASE}</span></div>
